...
export const HostModal: React.FC<HostModalProps> = ({
  ...
  const handleSmartPasteChange = (val: string) => {
    ...
    if (parsed.host) {
      ...
      const nextAutoName = `${parsed.username}@${parsed.host}`;
      // Update session name if:
      // 1. Name is currently empty
      // 2. User has not manually customized the name
      // 3. Name matches the previous auto-generated name from a previous paste in the same modal session
      if (!name.trim() || !isNameManuallyEditedRef.current || name === lastAutoGeneratedNameRef.current) {
        setName(nextAutoName);
        lastAutoGeneratedNameRef.current = nextAutoName;
      }
            ...
            <div className="col-span-3">
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  isNameManuallyEditedRef.current = e.target.value.trim().length > 0;
                }}
